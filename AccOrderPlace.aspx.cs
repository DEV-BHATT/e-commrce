...
{
    public partial class AccOrderPlace : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["id"] != null)
                {
                    if (Session["Buyitems2"] == null)
                    {
                        ...
                        string query = "select *from accessories where productId=" + Request.QueryString["id"];
                    ...
                    {
                        dt = (DataTable)Session["Buyitems2"];
                        int sr;
                        sr = dt.Rows.Count;
                        dr = dt.NewRow();
                        string con = "Data Source=DESKTOP-69CFBJ2;Initial Catalog=dream;Integrated Security=True";
                        SqlConnection scon = new SqlConnection(con);
                        string query = "select *from accessories where producteId=" + Request.QueryString["id"];
                        SqlCommand cmd = new SqlCommand();
                        cmd.CommandText = query;
                        cmd.Connection = scon;
                        SqlDataAdapter da = new SqlDataAdapter();
